...
export class StatsService {
  ...
  deletePlayerStats(playerId,statId) {
    ...
    this.http.delete<{message:string; data:any}>(url)
    .subscribe({
      next: data => {
  ...
  //   }
  // }
  updateManyStats(stats,season) {
    const statsObj = {
      stats,
      season
    }
    console.log("UPDATE MANY STATS");
    console.log(statsObj);
    let url = "http://localhost:3000/api/statistics/players/";
